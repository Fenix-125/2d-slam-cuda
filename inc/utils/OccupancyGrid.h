...
class OccupancyGrid {
    struct spokesGrid_t {
        std::vector<std::vector<size_t>> xGrid, yGrid, bearingIdxGrid;
        std::vector<std::vector<double>> rangeIdxGrid;
    };
...
public:
    ...
    spokesGrid_t spokesGrid() {
        ...
        {
            const std::vector<std::vector<size_t>> tmp_bearingIdxGrid{bearingIdxGrid};
            for (size_t j = 0; j < bearingIdxGrid.size(); ++j) {
                for (size_t i = 0u; i < numHalfElem; ++i) {
                    bearingIdxGrid[j][i] =
                            tmp_bearingIdxGrid[bearingIdxGrid.size() - j][bearingIdxGrid[0].size() - i] + numSpokes / 2;
                }
            }
        }

        for (size_t i = numHalfElem + 1u; i < 2u * numHalfElem + 1u; ++i) {
            bearingIdxGrid[numHalfElem][i] = numSpokes / 2;
        }
        auto rangeIdxGrid = np::sqrt_2d(np::add_2d(np::pow_2d(xGrid), np::pow_2d(yGrid)));
        return spokesGrid_t{xGrid, yGrid, bearingIdxGrid, rangeIdxGrid};
    }
